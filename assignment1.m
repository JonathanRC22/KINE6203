...
%
% Submitted by: Jonathan Radilla Cortez
%
...
%
% Answer: Removes all items from the workspace
% 
...
prefs = iptgetpref
% I had trouble finding this one
...
plot(x,y)
plot(linspace(-2*pi,2*pi),sin(x))        %<- command goes here
...
%
% Answer: Vartest - Chi-square variance test
%
...
% For statements loop a specific number of times, and keep track 
...
%
% Answer: NaN stands for Not a Number. NaN would be given as an answer when the
% answer is a not real or complex number (0/0 = NaN)
...
S = sum(A)
S = sum(A,'omitnan') - %This excludes the NaN's from the sum
S = sum(A,'omitnan') = 15
...
vecA = 2 3 4 5 6 7 
vecB = 1.1000
vecC = [] -% 1x0 empty double row vector was given as the asnwer
...
unitVector = 0.4243 0.7071 0.5657
